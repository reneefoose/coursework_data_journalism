...
* Take a look at the [Cheatsheets](https://www.rstudio.com/resources/cheatsheets/)
and [Tidyverse documentation](https://www.tidyverse.org/).
...
buyer_population <- read_csv('data/state_population_per_year.csv')
```
...
## Analysis
#All are showing different information.  Each data set are adding additional variables.
```
...
````{r}
##This is to determine the pharmacy with the most pills sent
#pharmacy_locations <- buyer %>%
#  group_by(buyer_address, buyer_annual, buyer_population, buyer_totals) %>%
 # summarise(
...
```{r}
## Florida had the most pills sent to it over the period of 2016-2012 at a fast glance of the data set.  I will confirm with writing a code to verify what my quick visual suggests.
