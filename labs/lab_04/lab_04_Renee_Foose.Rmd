...
```{r}
#creating a new column with the full address of the distributor based on the primary address, city, state and zip. The paste0() function helps us concatenate spaces and commas between the address lines 
...
```{r}
#using if_else() since there is only a binary condition. Would be ideal to use case_when() when there were nested if_else()
mingo_working = mingo %>%
...
#added one addition line of median
mingo %>%
...
```{r}
#use groupby on multiple factors and summarize the count. After than continue with the sort using the arrange(). arrange(), by default is in ascending order so you will have to manually do it desc() to make it descending

mingo %>%
 group_by(revised_company_name, reporter_name, buyer_name) %>%
 summarise(number_of_pills_per_grouping = n()) %>%
 arrange(desc(number_of_pills_per_grouping))
```
...
```{r}
#filtered the dataset with only the relevant row values and then grouped the data with their respected cities. Since there are significant rows we shall apply the distinct() function to remove the duplicates. Had their been no duplicates the data would have gone wild and we would not be able to prove that these companies are the same 
mingo %>%
  filter(reporter_name == "CARDINAL HEALTH" |  
         reporter_name == "CARDINAL HEALTH, INC." |
         reporter_name == "CARDINAL HEALTH 110, LLC") %>%
  group_by(reporter_name) %>%
  select(reporter_name, reporter_city) %>%
  distinct() 
```
...
My guess would be to change the other names of the Cardinal health facilities.  Use case_when() function where "CARDINAL HELATH, INC."  and change to CARDINAL HEALTH.  Or use when all the Cardianl facilities are the same company.  We can also use "if_else" where there is CARDINAL rows.
