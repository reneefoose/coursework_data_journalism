...
```{r}
average_county_pills_per_year = read.csv("https://raw.githubusercontent.com/smussenden/spring20-data-journalism/master/labs/lab_05/data/county_pills_per_year.csv")
...
```{r}
name = acs_variables %>%
  filter(label == "Estimate!!Total!!White alone") %>%
  select(name)

state_median_household_income <- get_acs(geography = "state", 
              variables=name$name, year=2014, geometry = FALSE )
...
opioid_states_scrape = opioid_states_scrape %>%
  html_nodes('table') %>%
  html_table(header=1, fill=TRUE)  %>%
  as.data.frame()
